...
import (
	...
	"github.com/trevorpiltch/omnifocus-sync/internal"
	"github.com/trevorpiltch/omnifocus-sync/internal/delta"
	"github.com/trevorpiltch/omnifocus-sync/internal/gh"
	"github.com/trevorpiltch/omnifocus-sync/internal/omnifocus"
  "github.com/trevorpiltch/omnifocus-sync/internal/shortcut"
)

// Version can be overridden at build time us_VERSION in the makefile.
...
func main() {
  ...
  desiredSHState, err := shortcut.GetStories(c.APIKey, c.Owner)
  if err != nil {
    log.Fatal(err)
  }

	log.Printf("Current state: %d issues; %d PRs; %d notifications %d stories.", len(currentState.Issues), len(currentState.PRs), len(currentState.Notifications), len(desiredSHState))
  ...
  d = delta.Delta(toSetSH(desiredSHState), toSetOF(currentState.Stories))
  log.Printf("Found %d changes to apply to Stories", len(d))
  for _, d := range d {
    if d.Type == delta.Add {
      err := og.AddStory(*(d.Item.(*shortcut.ShortcutStory)))
      if err != nil {
        // should never fail
        log.Fatal(err)
      }
...
func toSetSH(l []shortcut.ShortcutStory) map[delta.Keyed]struct{} {
  r := map[delta.Keyed]struct{}{}
  for _, i := range l{
    r[&shortcut.ShortcutStory {
      Title: i.Title, 
      HTMLURL: i.HTMLURL,
      K: i.K,
    }] = struct{}{}
...
func GetOFState(og omnifocus.Gateway) (OFCurrentState, error) {
  ...
  ofState.Stories, err = og.GetStories()
  if err != nil {
    return OFCurrentState{}, err
  }
